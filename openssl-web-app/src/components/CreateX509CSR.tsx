...
export default function CreateX509CSR({ formData, onFormDataChange, privKey }) {
    const handleSubmit = async (e) => {
        e.preventDefault();
        callOpenSSL('createCSR', privKey, formData, 'Error creating CSR')
            .then(data => {
                copyToClipboard(data.result, 'CSR copied to clipboard');
            })
            .catch(error => {
    ...
    return (
        <div className="p-4 bg-white bg-opacity-90 rounded shadow">
            <p className="text-lg font-medium text-gray-700">Create a Certificate Signing Request with your connected key</p>
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                    <label htmlFor="validity" className="block text-sm font-medium text-gray-700">Validity (days)</label>
                    <input 
                        type="number" 
                        id="validity" 
                        value={formData.validity} 
                        onChange={(e) => onFormDataChange('validity', e.target.value)} 
                        min={1} 
                        max={3650} 
                        className="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 text-gray-700"
                        required
                ...
                <button type="submit" className="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">Copy CSR to clipboard</button>
            </form>
        </div>
    );
}